...
# 에시) 인덱스 1의 listed_in 열 값 : International TV Shows, TV Dramas, TV Mysteries
genres = netflix['listed_in'].str.split(', ', expand=True).stack().value_counts()
genres
...
# 파이썬 리스트로 바꾼 country 열의 값이 explode( ) 함수를 적용하여 개별 행으로 분리
...
netflix_age_country = netflix_age_country.groupby('age_group')['country'].value_counts().unstack()